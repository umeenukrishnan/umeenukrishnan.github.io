...
The parameter $\beta > 0$ controls the nonlinearity of the filtered variable. For $\beta —— 0$ we have a linear behavior between physical and filtered variables, whereas $\beta — infinity $, we have a Heaviside step function. the value of $\beta$ plays an important role in the accuracy of the filtered result.

The primary variable which is of our concern should be either zero or 1. It is not desired to get a value that is in between 0 - 1. For $\sigma = 1$.
